...
export const qualityPhilosophyPosts: BlogPost[] = [
  {
    ...
    content: [
      ...
      "Contrain the critical inputs to a valid range, and use a critiria based input selection approach for the tests input parameters. This approach enables the non-critical inputs to vary dynamically.",
      "This can be achieved by implementing a helper function that retrieves a valid test input record from a predefined dataset (for example, a JSON repository). One very good idea is to use production-aligned data that has been sanitized/anonymized for testing purposes. Additionally, AI suggested (telemetry informed) test scenarios should be implemented to archive a complete end-to-end test automation quality system. Morking and stubbing can be used for non-critical external dependencies, However, mocking is not the ideal approach for the highest quality of signal-to-noise ratio. Additionally , realistic mocking data should be sources from the thrird party system's production data, where possible.",
      "",
      ...
      "* Language settings",
      "",
      "One key point: The QA team must be properly equipped and effective in analysing and diagnosing the root cause of the issue when tests fail. This is critical to continuously fine tune the dataset repository, and to avoid raising invalid defects, thereby increasing the signal-to-noise ratio which reduces confidence in the test suite. This is anyhow important for all test automation. QA Engineers can learn more about tools and methods to optimise signal-to-noise ratio in my [know your tools](/know-your-tools) video series.",
